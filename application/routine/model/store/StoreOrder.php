...
//suse app\routine\model\store\StoreCart;
//use app\routine\model\store\StoreOrderStatus;
...
class StoreOrder extends ModelBasic
{
    ...
    public static function cacheKeyCreateOrder($uid,$key,$addressId,$payType,$useIntegral = false,$couponId = 0,$mark = '',$combinationId = 0,$pinkId = 0,$seckill_id=0,$bargain_id=0)
    {
        ...
        $userInfo = User::getUserInfo($uid); // todo修改过

      //  $userInfo = AppUser::getUserInfo($uid);
        ...
        if($useIntegral && $userInfo['integral'] > 0 && $payPrice > 0){
            ...
            if($deductionPrice < $payPrice){
               ...
               // $res2 = false !== AppUser::edit(['integral'=>0],$userInfo['id'],'id');
            }else{
                $deductionPrice = $payPrice;
                $usedIntegral = bcdiv($payPrice,$other['integralRatio'],2);
               $res2 = false !== User::bcDec($userInfo['uid'],'integral',$usedIntegral,'uid');  //TODO 修改过
                //$res2 = false !== AppUser::bcDec($userInfo['id'],'integral',$usedIntegral,'id');
                $payPrice = 0;
            }
    ...
    public static function H5cacheKeyCreateOrder($uid,$key,$orderdata,$payType='weixin',$useIntegral = false,$couponId = 0,$mark = '',$combinationId = 0,$pinkId = 0,$seckill_id=0,$bargain_id=0)
    {
        ...
        if(!$uid) return  self::setErrorInfo('用户不存在!');
        $cartGroup = self::getCacheOrderInfo($uid,$key);
...
//        ]);
    ...
    public static function NewgetUserOrderDetail($uid,$key)
    {
        return self::where('order_id|unique',$key)->where('uid',$uid)->where('is_del',0)
            ->field('order_id,real_name,user_phone,user_address,cart_id,total_num,total_price,pay_price,pay_type,add_time,unique,cost,remark,deduction_price,coupon_price,paid')
            ->find();
    }
...
//        ],Url::build('My/order',['uni'=>$order['order_id']],true,true));
    ...
    public static function NewgetUserOrderList($uid,$status = '',$first = 0,$limit = 8)
    {
        $list = self::statusByWhere($status)->where('is_del',0)->where('uid',$uid)
            ->field('add_time,id,order_id,paid,status,pay_type,deduction_price,refund_status,pink_id,total_price')
            ->order('add_time DESC')->limit($first,$limit)->select()->toArray();
        foreach ($list as $k=>$order){
            $list[$k] = self::tidyOrder($order,true);
        }
    ...
    public static function NewsearchUserOrder($uid,$order_id)
    {
        $order = self::where('uid',$uid)->where('order_id',$order_id)->where('is_del',0)->field('id,order_id,total_postage,status,pay_type,deduction_price')
            ->order('add_time DESC')->find();
        if(!$order)
            return false;
        else
