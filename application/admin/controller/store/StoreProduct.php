...
use app\admin\model\store\StoreProductVedioFile;
...
//        return $this->fetch('public/common_form');
    ...
    public function edit($id)
    {
        ...
        $field = [
            ...
            Form::frameImageOne('video_img','产品视频',Url::build('admin/widget.images/index',array('fodder'=>'video_img')),$product->getData('video_img'))->icon('video_img')->width('100%')->height('500px'),
    ...
    public function update(Request $request, $id)
    {
        ...
        if($data['stock'] == '' || $data['stock'] < 0) return Json::fail('请输入库存');
        $data['image'] = $data['image'][0];
        $data['video_img'] = $data['video_img'][0];
        $data['slider_image'] = json_encode($data['slider_image']);
    ...
    * 创建3d模型提交页面
    */
    public function create3d(Request $request){
        ...
        if($type==3){//其它分类
            ...
            $config_tab = [["value"=> 1,"label"=>  "3d文件添加" ,"icon"=> "cog" ,"type"=> 0]] ;
        ...
        $res  = StroeProduct3dFile::where(['product_id'=>$id])->find();
    ...
    public function Mag(Request $request)
    {
        ...
        if ($_FILES['vedio_file']['tmp_name']) {
            ...
            if ($_FILES['vedio_file']['type'] == 'application/zip') {
                $key = '3d' . time() . '.zip';
            }
            list($ret, $err) = $uploadMgr->putFile($token, $key, $filePath);
            $vedio_name = $request->post('vedio_name');
            $model = new StroeProduct3dFile();
            $model->product_id = $id;
            $model->file_name = $key;
            $model->save();
            if (!$model->save()){
                $this->success('新增成功', 'store.store_product/index?type=1',[],1);
            }else{
                $this->error('新增失败');
            }
        }
...
//     * */
   ...
   * 创建3d模型提交页面
   */
    public function createvedio(Request $request)
    {
        $type = input('type') != 0 ? input('type') : 0;
        $tab_id = 16;
        if (!$tab_id) $tab_id = 1;
        ...
        if ($type == 3) {//其它分类
            ...
            $config_tab = [["value" => 1, "label" => "商品视频添加", "icon" => "cog", "type" => 0]];
        ...
        foreach ($list as $k => $v) {
            ...
            $id = $request->param('id', 0);
            $res = StoreProductVedioFile::where(['product_id' => $id])->find();
  ...
  * 创建商品视频操作
  */
    public function Magvedio(Request $request)
    {
        $id = $request->param('id');
        $post = $request->post();
        ...
        if (!$_FILES['vedio_file']['tmp_name']) {
            //$this->error('文件不能为空', null, [], 1);
            $model = StoreProductVedioFile::where(['product_id'=>$id])->delete();
            if (!empty($model)){
                $this->success('修改成功', 'store.store_product/index?type=1',[],1);
            }
        }
        ...
        if ($_FILES['vedio_file']['tmp_name']) {
            ...
            list($ret, $err) = $uploadMgr->putFile($token, $key, $filePath);
            $vedio_name = $request->post('vedio_name');
            $model = new StoreProductVedioFile();
            $model->product_id = $id;
            $model->vedio_url = config('qn.url').'/'.$key;
            $model->save();
            if (!$model->save()){
                $this->success('新增成功', 'store.store_product/index?type=1',[],1);
            }else{
