...
class StoreCategory extends ModelBasic
{
    ...
    {
        $list = self::where(['pid'=>$pid,'is_show'=>1])
            ->order('sort', 'desc')
     ...
     * 获取分类下的商品
     */
    public function SonCat($cat_id)
    {
        $data = self::where(['pid'=>$cat_id])->where(['is_show'=>1])->order('sort', 'asc')->field(['id', 'cate_name'])->select();
        $res  = [];
        if (!empty($data)){
            $ProductModel = new StoreProduct();
            foreach ($data as $key => $value){
               $res['好物精选'] =  $ProductModel->GoodBest($cat_id);
               $res[$value['cate_name']] = $ProductModel ->GetGood($value['id']);
            }
        }
        return $res;
    }
