...
//                $model = $model->whereOr('p.cate_id','LIKE',["%$catid%",$catidab]);
     ...
     * 获取热销商品
     */
    public function Hot($cat_id=0)
    {

        if (empty($cat_id)) {
            $list = self::where(['is_show' => 1, 'is_hot' => 1])
                ->order('sort', 'desc')
                ->field(['id', 'store_name', 'image', 'store_info', 'price'])
                ->paginate(4);
        }else{
            $CateData = StoreCategory::get($cat_id);
            if ($CateData['pid'] == 0){
                $cate_id_data = StoreCategory::where(['pid'=>$CateData['id']])->select();
                $cat_id = [];
                if (!empty($cate_id_data)){
                    foreach ($cate_id_data as $val){
                        $cat_id[] = $val['id'];
                    }
                }
            }
    ...
    * 获取推荐商品
    */
    public function Best($cat_id=0)
    {
        if (empty($cat_id)) {
            $list = self::where(['is_show' => 1, 'is_best' => 1])
                ->order('sort', 'desc')
                ->field(['id', 'store_name', 'image', 'store_info', 'price'])
                ->paginate(10);
        }else{
            //$list = self::where(['is_show' => 1, 'is_best' => 1,'cate_id'=>$cat_id])
            $list = self::where(['is_show' => 1, 'is_best' => 1])
                ->order('sort', 'desc')
                ->field(['id', 'store_name', 'image', 'store_info', 'price'])
                ->paginate(10);
     ...
     * 热销商品
     */
    public function GetGood($cat_id=0)
    {
        if (empty($cat_id)) {
            $data = self::where(['is_show' => 1])->field(['id', 'image', 'store_name', 'store_info', 'price'])->paginate(10);
        }else{
            //根据一级分类获取到一级分类下的二级分类
            $TwoCat = StoreCategory::where(['pid'=>$cat_id,'is_show'=>1])->field(['id'])->select();
            $cat_id_data = [];
            if (empty($TwoCat->toArray())){
                $cat_id_data[] = $cat_id;
            }else{
            ...
            $data = self::where('cate_id','in',$cat_id_data)->where(['is_del'=>0,'is_hot'=>1])->field(['id','image','store_name','store_info','price'])->paginate(10);
        }
        return $data;
    }
    ...
    public function GoodBest($cat_id=0)
    {
        $list = [];
        $cat_data = \app\admin\model\store\StoreCategory::where(['pid'=>$cat_id])->where(['is_show'=>1])->field('id')->select();
        if (!empty($cat_data)) {
            $ids = [];
            ...
            $list = self::whereIn('cate_id', $ids)
                ->where(['is_show' => 1, 'is_best' => 1])
                ->order('sort', 'desc')
                ->field(['id', 'store_name', 'image', 'store_info', 'price'])
                ->select();
        }
    ...
    public function GoodLists($name='',$type='')
    {
        switch ($type){
            case 'price_desc':
                $par = ['filed'=>'price','sort'=>'desc'];
                break;
            case 'price_asc':
                $par = ['filed'=>'price','sort'=>'asc'];
                break;
            case 'sale_desc':
                $par = ['filed'=>'sales','sort'=>'desc'];
                break;
            case 'sale_asc':
                $par = ['filed'=>'sales','sort'=>'asc'];
                break;
            default:
                $par = ['filed'=>'sort','sort'=>'asc'];
        }


        $list = self::where(['is_show' => 1])
            ->where('store_name','like','%'.$name.'%')
            ->order($par['filed'], $par['sort'])
            ->field(['id', 'store_name', 'image', 'store_info', 'price','sales'])
            ->paginate(4);
